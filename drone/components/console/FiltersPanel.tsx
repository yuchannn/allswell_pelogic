...
import { KIND_COLOR, KIND_ORDER } from "@/lib/species";
...
/** `grid` = every drone at equal size; `focus` = one large frame + a strip of the others. */
export type Layout = "grid" | "focus";

export interface ViewSettings {
  /** Detections inside this radius count as "in range" (list + status bar) */
  rangeNm: (typeof RANGE_OPTIONS)[number];
  layout: Layout;
  /** In focus layout, switch the large frame to the drone that just found a school */
  autoFocus: boolean;
      ...
      <div className="flex flex-col gap-2 text-[11.5px]">
        <Field label={t("layout")}>
          <div className="seg" role="group">
            <button type="button" aria-pressed={view.layout === "grid"} onClick={() => onChange({ layout: "grid" })}>
              {t("layout_grid")}
            </button>
            <button type="button" aria-pressed={view.layout === "focus"} onClick={() => onChange({ layout: "focus" })}>
              {t("layout_focus")}
            </button>
          </div>
        </Field>
        ...
        <Check checked={view.autoFocus} onChange={(v) => onChange({ autoFocus: v })} label={t("autoFocus")} />
      </div>
