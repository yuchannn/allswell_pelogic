...
export const STATUS_STYLE: Record<Drone["status"], string> = {
...
interface Props {
  state: SimState;
  /** Drone whose frame is highlighted / enlarged on the feed wall */
  focusedId: string | null;
  onFocus: (id: string) => void;
}

export function FleetPanel({ state, focusedId, onFocus }: Props) {
  const { t } = useLocale();
  return (
    <section className="border-b border-line p-3">
      ...
      <div className="flex flex-col gap-2">
        {state.drones.map((d, i) => (
          <DroneCard key={d.id} drone={d} index={i + 1} state={state} focused={d.id === focusedId} onFocus={() => onFocus(d.id)} />
        ))}
      </div>
    </section>
  );
...
function DroneCard({
  ...
  return (
    <div
      ...
      aria-label={`${d.callsign} · ${t("focusDrone")}`}
      title={`${t("focusDrone")} · ${index}`}
      onClick={onFocus}
      onKeyDown={(e) => {
      ...
      className={`cursor-pointer rounded-md border bg-panel-2 p-2.5 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-accent ${
        focused ? "border-accent-2/70 bg-panel-3" : "border-line hover:border-line-2"
      }`}
    >
...
export function LinkBars({ q }: { q: number }) {
